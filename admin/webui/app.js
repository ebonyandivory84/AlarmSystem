(() => {
  ...
  function getState(id){ return new Promise((res,rej)=>{ const cb=(a,b)=>{ const s=b||a; if(s&&typeof s==='object') res(s); else res(null); }; if(typeof state.socket.getState==='function') state.socket.getState(id,cb); else if (typeof state.socket.emit==='function') state.socket.emit('getState',id,cb); else rej(new Error('getState not available')); }); }
  ...
  async function loadTriggerLogForDate(day){
    ...
    triggerLogView.textContent = st?.val ? String(st.val) : `Keine Trigger-Logs für ${day}`;
  }
  ...
  function inferZoneByIdentity(id, key, label, fallback){
    const s = `${String(id || '')} ${String(key || '')} ${String(label || '')}`.toLowerCase();
    const isCamera = /(cam|camera|reolink|human.?detect|person.?detect)/.test(s);
    const isContact = /(door|window|fenster|tuer|tür|kontakt|contact|garage.*door)/.test(s);
    if (isCamera) return 'perimeter';
    if (isContact) return 'aussenhaut';
    return fallback;
  }
  ...
  function setEntity(kind, idx, patch){
    ...
    const next = { ...patch };
    if (typeof next.zone === 'string' && next.zone !== 'pool') {
      next.zone = zoneByKind(kind, next.zone);
    }
    Object.assign(state.config[kind][idx], next);
  }
  ...
  function loadProfile(){ const n=profileSelect.value; const p=state.profiles[n]; if(!p){setStatus(`Profil nicht gefunden: ${n}`,true);return;} state.activeProfile=n; state.config=clone(p); normalizeSemanticZones(); renderAll(); setStatus(`Profil geladen: ${n}`); }
